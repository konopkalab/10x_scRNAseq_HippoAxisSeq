...
umap_pl <- plotReducedDim(sce_milo, dimred = "umap", colour_by="Axis", text_by = "Cell", text_size = 3) +
  guides(fill="none")

## Plot neighbourhood graph
nh_graph_pl <- plotNhoodGraphDA(sce_milo, da_results, layout="umap",alpha=0.2)

pdf("Reviewer1/MILO_InhibitoryNeurons_Results.pdf",width=12,height=8)
umap_pl + nh_graph_pl +
  plot_layout(guides="collect")
dev.off()

pdf("Reviewer1/MILO_InhibitoryNeurons_Results_Graph.pdf",width=8,height=8)
print(nh_graph_pl)
dev.off()

da_results <- annotateNhoods(sce_milo, da_results, coldata_col = "Cell") 

openxlsx::write.xlsx(da_results, file = "Reviewer1/Cell_Milo_InhibitoryNeurons_Results.xlsx", colNames = TRUE, borders = "columns")

pdf("Reviewer1/MILO_InhibitoryNeurons_Results_Beeswarm.pdf",width=6,height=8)
plotDAbeeswarm(da_results, group.by = "Cell",alpha = 0.2)
dev.off()
