...
openxlsx::write.xlsx(res_model, file = "Reviewer1/Inhibitory_Neurons_proportion_statistics.xlsx", colNames = TRUE, borders = "columns")

# Visualize the odds ratios vs -log10(FDR)
pdf("Reviewer1/AnteriorVsPosterior_InhibitoryNeuronsAbundance.pdf",width=5,height=5)
ggplot(aes(x = odds.ratio, y = -log10(FDR)), data = res_model) +
  ...
  scale_x_log10() + 
  theme_minimal() + 
  labs(x = 'Anterior / Posterior', title = 'Neurons subtypes proportion change') + 
  geom_vline(xintercept = 1, linetype="dashed", color = "blue", size=1.5)
dev.off()

# Visualize the abundance by boxplot
pdf("Reviewer1/AnteriorVsPosterior_InhibitoryNeuronsAbundance_Boxplot.pdf",width=8,height=8)
...
openxlsx::write.xlsx(prop, file = "Reviewer1/Inhibitory_Neurons_proportion_statistics_speckle.xlsx", colNames = TRUE, borders = "columns")
